...
export default function App() {
  ...
  const [hiddenTasks, setHiddenTasks] = useState(() => {
    try { return JSON.parse(localStorage.getItem("mio-hidden-tasks") || "{}"); }
    catch { return {}; }
  });
  ...
  function getOrderedTasks(day) {
    ...
    const pool = [...incoming, ...staying, ...custom].filter(t=>!hiddenTasks[t.id]);
  ...
  const todayTasks = IS_MARCH_2026 ? (() => {
    const base = ALL_BASE_TASKS.filter(t=>!hiddenTasks[t.id]).filter(t=>{ const md=moved[t.id]; if(md) return md===TODAY_DAY; return (BASE_TASKS_BY_DAY[TODAY_DAY]||[]).find(b=>b.id===t.id); });
                              ...
                              return(
                                ...
                                >
                                  ...
                                  <div style={{display:"flex",gap:4,flexShrink:0,alignItems:"center"}}>
                                    {/* ⭐ 중요표시 — 중요한 것만 표시, editMode일 때 토글 가능 */}
                                    {important[task.id]&&<span style={{fontSize:15,lineHeight:1}}>⭐</span>}
                                    {/* 삭제 버튼 — editMode일 때만 */}
                                    {editMode&&isCustom&&<button onClick={()=>deleteCustomTask(day,task.id)} style={{border:"none",background:"#fee2e2",color:"#ef4444",borderRadius:8,padding:"3px 7px",fontSize:11,fontWeight:700,cursor:"pointer"}}>삭제</button>}
